const db = require('../database/DB_Prestadores.js');

module.exports = {
  async insert(req, res) {
    try {
      await db.insert(req.body.id_condominio, req.body.prestador);
      return res.json();
    } catch (err) {
  ...
  async update(req, res) {
    try {
      await db.update(req.body.id_condominio, req.body.prestador);
      return res.json();
    } catch (err) {
      return res.status(500).json({ message: err.message });
    }
