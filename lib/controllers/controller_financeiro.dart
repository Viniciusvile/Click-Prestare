...
apiGetAllFinanceiro(String route, String mes, String ano) async {
  var url = Uri.http(link, '/'+route+'/get-all',{'id_condominio': Singleton.instance.id_condominio.toString(), 'mes':mes, 'ano':ano});
  try{
...
apiGetDetailsInadimplente(String route, String bloco, String apto) async {
  var url = Uri.http(link, '/'+route+'/get',{'id_condominio': Singleton.instance.id_condominio.toString(), 'bloco': bloco, 'apto': apto});
  try{
      var response = await http.get(
        url,
        headers: { "Authorization": getToken() }
