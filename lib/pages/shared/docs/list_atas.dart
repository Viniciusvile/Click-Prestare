...
class _ListAtasPageState extends State<ListAtas> {
  ...
  loadList() async{
    try{
     _isLoading = true;
      setState(() {});
      var locals = await apiGetAllDocs("documentos",1);
      list = locals;
      _isLoading = false;
      setState(() {});
    }catch(e){
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Container(  
            ...
            child: Column(
              children: [
                NavigationDefault(title: getText('docs_nav_atas')),
                Flexible(
                  child: Container(
      ...
      floatingActionButton: (getUserType() == 'sindico') ?
        FloatButton(onPressed: () { 
          Navigator.push(context,MaterialPageRoute(builder: (context) => NewDocument(is_ata: true)),).then((_) {
              loadList();
            });
          },)
        : null
