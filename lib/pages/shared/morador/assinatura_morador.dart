...
class _AssinaturaMoradorPageState extends State<AssinaturaMorador> {
  ...
  String getDiasRestantes(){
    if(Singleton.instance.dias_restantes_morador < 0){
      return "0 dias";
    }
    if(Singleton.instance.dias_restantes_morador == 1){
      return "1 dia";
    }
    return "${Singleton.instance.dias_restantes_morador.toString()} dias";
  }
  ...
  buy() async{
    ...
    //     }
    //   } else {
    //     displayMessage(context, getText('alert_error'), getText('assinatura_erro_loja'));
    //   }    
    // }catch(e){
  ...
  loadProducts() async { 
    ...
    //   final bool available = await InAppPurchase.instance.isAvailable();
    //   if (available) {
    //     ProductDetailsResponse response = await InAppPurchase.instance.queryProductDetails({'click_mensal_morador','click_semestral_morador','click_anual_morador'});                                        
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 20),
                          DividerDefault(title: getText('assinatura_morador'), fontSize: 16, align: TextAlign.center, height: 39,), 
                          SizedBox(height: 10),
                          Row(
                            children: [
                              CircleAvatar(                      
                          ...
                          Row(
                            children: [
                              LabelDefault(title: "${getText('nome')}: ", weight: FontWeight.w500, size: 16, color: Colors.grey.shade600,),
                              LabelDefault(title: getUsername(), size: 15, color: Colors.black,),
                            ],
                          ),
                          SizedBox(height: 10),                         
                          // Row(
                          ...
                          Row(
                            children: [
                              ...
                              LabelDefault(title: Singleton.instance.vencimento_morador, size: 15, color: Colors.black,),
                            ],
                          ),
                          SizedBox(height: 10),
                          Row(
                          ...
                          SizedBox(height: 20), 
                          if(Singleton.instance.dias_restantes_morador <= 7) 
                            Column(
                              children: [
                                if(Singleton.instance.dias_restantes_morador <= 0)
                                  Padding(
                                ...
                                LabelDefault(title: getText('assinatura_renove_agora'), size: 15, maxLines: 10,),
                                SizedBox(height: 5),
                                LabelDefault(title: getText('assinatura_renove_agora_obs'), size: 15, maxLines: 10,),
                                SizedBox(height: 20),                                
                                Column(
                                  children: [
                                    ...
                                    SizedBox(height: 10), 
                                    DefaultButton(title: getText('assinatura_btn_mensal_morador'), hasArrow: false,                   
                                      onPressed: () {
                                        buy();
                                      }
                                    ),
